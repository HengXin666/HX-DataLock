...
```python
from hx_datalock import open_file, send_file, send_file_with_public_doc

send_file("public.hxdl.json", "plain.txt", "plain.txt.hxdl.json")
...
### `send_file(public_key_document_path, input_path, output_path)`

使用 Public Key Document 加密本地文件并写出 Data Envelope。这个 helper 是发送方 API, 会拒绝完整 Keyring。

### `send_file_with_public_doc(public_key_document_path, input_path, output_path)`

`send_file` 的显式命名别名。推荐在新代码中使用这个名称, 避免把完整 Keyring 传入发送方环境。
